...
object MovieSimilaritiesDataset {
  ...
  def main(args: Array[String]) {
    ...
    // self-join = ratings.as("ratings1").join(ratings.as("ratings2")
...
//    cache()是暂存的意思
    ...
    if (args.length > 0) {
      ...
      val filteredResults = moviePairSimilarities.filter(
        (col("movie1") === movieID || col("movie2") === movieID) // movie1和movie2没有主次关系
          &&
          col("score") > scoreThreshold
          &&
          col("numPairs") > coOccurrenceThreshold
      )
